...
class User {
    ...
    static async saveMedicalProfessional(professionalJSONDATA) {
        ...
        try {
            ...
            const { rows } = await db.pool.query('SELECT saved_professionals FROM users WHERE user_id = $1', [user_id]);
            ...
            const isDuplicate = savedProfessionalsArray.some(
                (item) => JSON.parse(item).professional_id === professional.professional_id
            );
            ...
            const updatedProfessionalsArray = [...savedProfessionalsArray, jsonString];
    
            // Execute the SQL query to update the column with the updated JSON data array
            await db.pool.query(
                'UPDATE users SET saved_professionals = $1 WHERE user_id = $2',
    ...
    static async getSavedMedicalProfessionals(user_id) {
        ...
        try {
            ...
            if (result.rows.length > 0) {
                const savedProfessionalsArray = result.rows[0].saved_professionals || [];
                const savedProfessionalsObjects = savedProfessionalsArray.map((item) => JSON.parse(item));
    ...
    static async deleteSavedMedicalProfessional(user_id, professional_id) {
        try {
            ...
            // Return some success message or result if needed
            return { success: true, message: "Professional data deleted successfully" };
        } catch (error) {
            // Handle any database or other errors that may occur during the query
